...
from accessorymanager import PlayerAccessory
from sis import SIS
from sismanager import PlayerSIS
...
class TeamSlot:
    ...
    def equip_sis(self, sis_entry: PlayerSIS) -> bool:
        ...
        if sis_entry.sis.slots <= self.available_sis_slots:
            ...
            warnings.warn(f"Cannot equip SIS '{sis_entry.sis.name}': Not enough slots. "
    ...
    def __repr__(self) -> str:
        ...
        if self.sis_list:
            ...
            sis_items = [f"    - {sis.name} ({sis.slots} slots)" for sis in self.sis_list]
