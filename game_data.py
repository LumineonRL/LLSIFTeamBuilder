...
class GameData:
    ...
    def _load_combo_bonuses(
        ...
        try:
            ...
            warnings.warn(
                f"Could not load combo bonuses from '{filepath}': {e}."
            )
            return [(0, 1.0)]
    ...
    def _load_note_speed_map(self, filepath: str) -> Dict[int, float]:
        ...
        try:
            ...
            warnings.warn(
                f"Could not load note speed map from '{filepath}': {e}."
            )
            # Provide a default mapping if file is missing/corrupt
            return {
                i: (1.6 - i * 0.1) if i >= 6 else (1.9 - i * 0.15)
                for i in range(1, 11)
            }

    def _load_combo_fever_map(
        self, filepath: str
    ) -> List[Tuple[int, float]]:
        ...
        try:
            ...
            warnings.warn(
                f"Could not load combo fever map from '{filepath}': {e}."
            )
            return []
