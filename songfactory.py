...
class SongFactory:
    ...
    def _index_song_data(self, raw_data: Dict[str, Any]) -> None:
        ...
        for song_id_json, record in raw_data.items():
            try:
                ...
                warnings.warn(f"Warning: Skipping invalid song record with key '{song_id_json}': {e}")
    ...
    def create_song(self, identifier: Union[str, Tuple[str, str]]) -> Optional[Song]:
        ...
        if isinstance(identifier, str):
            ...
            warnings.warn(f"Invalid identifier type: {type(identifier)}. Must be str or (str, str) tuple.")
            return None
