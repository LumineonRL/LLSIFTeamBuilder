...
class Play:
    ...
    def __init__(self, team: Team, song: Song, config: PlayConfig,
                 game_data: GameData):
        """
    ...
    def simulate(self, n_trials: int = 1,
        ...
        for i in range(n_trials):
            ...
            if self.logger:
                self.logger.info("--- Simulation Trial %d/%d ---", i + 1,
                                 n_trials)
    ...
    def calculate_ppn_for_all_slots(
        ...
        if team_total_stat == 0:
            warnings.warn(
                "Team total stat for song attribute is 0. All PPN will be 0."
            )
    ...
    def _setup_logger(self, log_level: int) -> logging.Logger:
        ...
        log_filename = (f"{sanitized_title.replace(' ', '_')}_"
    ...
    def _log_trial_summary(self, trial: Trial, total_uptime: float):
        ...
        ratio_percent = ((trial.perfect_hits / total_judgements * 100)
                         if total_judgements > 0 else 0)
        self.logger.info(
            "Perfect Ratio: %d / %d (%.2f%%)",
            trial.perfect_hits, total_judgements, ratio_percent,
        )
        uptime_percent = ((total_uptime / self.song.length * 100)
                          if self.song.length > 0 else 0)
        self.logger.info(
            "Perfect Lock Uptime: %.2fs (%.2f%%)", total_uptime,
            uptime_percent
        )

    def _log_overall_summary(self, scores: List[int],
                             uptimes: List[float]):
        ...
        self.logger.info(
            ...
            np.mean(uptimes), avg_uptime_percent,
        )

    def __repr__(self) -> str:
        """Provides a summary of the Play configuration."""
        team_stats = (f"{self.team.total_team_smile}/"
