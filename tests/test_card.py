...
class TestCardFactory(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gallery_bonus = Gallery(0, 0, 0)
    ...
    def test_initialize_card(self):
        test_card = self.factory.create_card(101, self.gallery_bonus)
        self.assertEqual(
            f"Created card: {test_card.display_name} ({test_card.rarity})",
            "Created card: Initial Nozomi (UR)",
        )
    ...
    def test_initialize_idolized(self):
        test_card = self.factory.create_card(101, self.gallery_bonus, idolized=True)
        self.assertEqual(
            f"Idolization Status: {test_card.idolized_status}",
            "Idolization Status: idolized",
        )

    def test_card_out_of_range(self):
        test_card = self.factory.create_card(11101, self.gallery_bonus)
        self.assertIsNone(test_card)

    def test_modify_ur_level(self):
        test_card = self.factory.create_card(
            101, self.gallery_bonus, idolized=True, level=200
        )
        self.assertEqual(f"Smile: {test_card.stats.smile}", "Smile: 5990")

    def test_unidolized_promo(self):
        with self.assertWarns(UserWarning) as cm:
            test_card = self.factory.create_card(96, self.gallery_bonus, idolized=False)
            self.assertEqual(
                str(cm.warnings[0].message),
                "Card ID 96 is a promo and cannot be unidolized. Forcing to idolized state.",
            )
        self.assertIsNotNone(test_card)
        self.assertEqual(
            f"Created card: {test_card.display_name} ({test_card.rarity})",
            "Created card: Season 1 BD Bonus Maki (UR)",
        )

    def test_modify_promo_level(self):
        test_card = self.factory.create_card(
            96, self.gallery_bonus, idolized=True, level=101
        )
        self.assertEqual(f"Smile: {test_card.stats.smile}", "Smile: 3925")

    def test_modify_skill_level(self):
        test_card = self.factory.create_card(101, self.gallery_bonus, skill_level=5)
        self.assertEqual(
            f"Skill Chance: {test_card.skill_chance}", "Skill Chance: 0.52"
        )

    def test_skill_level_out_of_range(self):
        with self.assertWarns(UserWarning) as cm:
            test_card = self.factory.create_card(
                101, self.gallery_bonus, skill_level=20
            )
            self.assertEqual(
                str(cm.warnings[0].message),
                "Invalid skill level '20'. Must be between 1 and 8. Defaulting to 1.",
            )
        self.assertEqual(
            f"Skill Chance: {test_card.skill_chance}", "Skill Chance: 0.36"
        )

    def test_modify_sis_slots(self):
        test_card = self.factory.create_card(
            101, self.gallery_bonus, idolized=True, sis_slots=6
        )
    ...
    def test_sis_slots_out_of_range(self):
        with self.assertWarns(UserWarning) as cm:
            test_card = self.factory.create_card(
                101, self.gallery_bonus, idolized=True, sis_slots=3
            )
            self.assertEqual(
                str(cm.warnings[0].message),
                "Invalid SIS slots value '3'. Must be between 4 and 8. Defaulting to 4.",
            )
    ...
    def test_non_integer_level(self):
        with self.assertWarns(UserWarning) as cm:
            test_card = self.factory.create_card(
                101, self.gallery_bonus, idolized=True, level=101.5
            )
            self.assertEqual(
                str(cm.warnings[0].message),
                "Invalid type for level: got float, expected int. Ignoring custom level.",
            )
    ...
    def test_get_skill_for_arbitrary_level(self):
        ...
        test_card = self.factory.create_card(
            1021, self.gallery_bonus, idolized=True, skill_level=5
        )
        self.assertIsNotNone(test_card)

        chance_at_lvl_11 = test_card.get_skill_attribute_for_level(
            test_card.skill.chances, 10
        )
