...
class TestCardFactory(unittest.TestCase):
    ...
    def test_modify_ur_level(self):
        test_card = self.factory.create_card(101, self.gallery_bonus, idolized=True, level=200)
    ...
    def test_modify_promo_level(self):
        test_card = self.factory.create_card(96, self.gallery_bonus, idolized=True, level=101)
    ...
    def test_skill_level_out_of_range(self):
        with self.assertWarns(UserWarning) as cm:
            test_card = self.factory.create_card(101, self.gallery_bonus, skill_level=20)
    ...
    def test_modify_sis_slots(self):
        test_card = self.factory.create_card(101, self.gallery_bonus, idolized=True, sis_slots=6)
        self.assertEqual(f"SIS Slots: {test_card.current_sis_slots}", "SIS Slots: 6")

    def test_sis_slots_out_of_range(self):
        with self.assertWarns(UserWarning) as cm:
            test_card = self.factory.create_card(101, self.gallery_bonus, idolized=True, sis_slots=3)
    ...
    def test_non_integer_level(self):
        with self.assertWarns(UserWarning) as cm:
            test_card = self.factory.create_card(101, self.gallery_bonus, idolized=True, level=101.5)
    ...
    def test_get_skill_for_arbitrary_level(self):
        ...
        test_card = self.factory.create_card(1021, self.gallery_bonus, idolized=True, skill_level=5)
        self.assertIsNotNone(test_card)
